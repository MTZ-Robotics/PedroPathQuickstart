...
import static pedroPathing.MTZ.mtzConstants_ItD.maxArmExtendInches;
...
import static pedroPathing.MTZ.mtzConstants_ItD.minArmExtendInches;
...
import static pedroPathing.MTZ.mtzConstants_ItD.ticksPerInchExtend;
...
import pedroPathing.MTZ.mtzButtonBehavior;
import pedroPathing.MTZ.mtzConstants_ItD;
...
/****Green*********           Center Stage R1     Controls Update Status           *******Green*******/
            ...
            if(hasAuxMotorsAndServos) {
/*
                armExtendInches = extend.getCurrentPosition()/ticksPerInchExtend;
                if(armExtendInches < minArmExtendInches){
                    //Don't listen to request to retract more
                    if(-armExtendStick>0){
                        extend.setPower(defaultArmPower * powerRatio * (-armExtendStick));
                    } else {
                        extend.setPower(0);
                    }
                } else if(armExtendInches > maxArmExtendInches){
                    //Don't listen to request to extend more
                    if(-armExtendStick<0){
                        extend.setPower(defaultArmPower * powerRatio * (-armExtendStick));
                    } else {
                        extend.setPower(0);
 ...
 */
                ...
                //Limit the rearward reach of the arm to stay within 42"
                if (handVerticalStick < 0) {
                    if(armRotationDegrees<120 || armExtensionInches< (armExtensionInchesAtHome + 1) || powerRatio>1.0) {
            ...
            if(hasLightsHub) {
               ...
               if (endGameTimer.seconds() > endGameOver) {
                ...
                if(Objects.equals(lightsColor, "green")){
                    greenLED1.setMode(DigitalChannel.Mode.INPUT);
                    redLED1.setMode(DigitalChannel.Mode.OUTPUT);
                    greenLED2.setMode(DigitalChannel.Mode.INPUT);
                    redLED2.setMode(DigitalChannel.Mode.OUTPUT);
                } else if(Objects.equals(lightsColor, "yellow")){
                    greenLED1.setMode(DigitalChannel.Mode.OUTPUT);
                    redLED1.setMode(DigitalChannel.Mode.OUTPUT);
                    greenLED2.setMode(DigitalChannel.Mode.OUTPUT);
                    redLED2.setMode(DigitalChannel.Mode.OUTPUT);
                } else if(Objects.equals(lightsColor, "red")){
                    greenLED1.setMode(DigitalChannel.Mode.OUTPUT);
                    redLED1.setMode(DigitalChannel.Mode.INPUT);
                    greenLED2.setMode(DigitalChannel.Mode.OUTPUT);
                    redLED2.setMode(DigitalChannel.Mode.INPUT);
                } else { //Black
                    greenLED1.setMode(DigitalChannel.Mode.INPUT);
                    redLED1.setMode(DigitalChannel.Mode.INPUT);
                    greenLED2.setMode(DigitalChannel.Mode.INPUT);
...
//Telemetry Methods
    public void displayTelemetry() {
        ...
        if(hasAuxMotorsAndServos) {
            ...
            telemetry.addLine()
                    .addData("Arm Extension Inches: ", armExtension.getCurrentPosition()/ticksPerInchExtension);
            telemetry.addLine()
